...
class customEnv():
    ...
    def render(self):
        """
            Method: How to print your ENV
        """
        if self.state == 0:
            return "The environment is in a state where \"there are no items in stock\"."
        if self.state == 1:
            return "The environment is in a state where \"there is one item in stock\"."
        if self.state == 2:
            return "The environment is in a state where \"there are two items in stock\"."
        # print("Current state of the stock is", self.state)
